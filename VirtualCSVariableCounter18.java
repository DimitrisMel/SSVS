...
public class VirtualCSVariableCounter18 {
	...
	public static void estimateSize(String filePath) throws FileNotFoundException {
		...
		Scanner sc = new Scanner(new File(filePath));
		String resultFilePath = GeneralUtil.path + "Results\\SSVS-2_VirtualCSVariableCounter18_" 
		...
		while (sc.hasNextLine()) {
			...
			if (true) {
				...
				if (numHashes > 8) {
					...
					estimate  = 0;
					if (minIndex <= numHashes/4-1) minIndex =0;
					else if(minIndex >= numHashes/2+1) minIndex = numHashes/2;
					else {minIndex -=1;}
					for (int i = minIndex; i<minIndex+numHashes/2; i++) {
						estimate += value[i];
					}
