...
public class RCS_AC {
	...
	public static ActiveCounter8[][] C;
	...
	public static final int d =512; 			// the number of rows in Count Min
	...
	public static int counterSize = 8;				// size of each unit
	...
	public static ActiveCounter8[][] generateCounter() {
		m = mValueCounter;
		u = counterSize * mValueCounter;
		w = (M / u) ;
		ActiveCounter8[][] B = new ActiveCounter8[1][w];
		for (int i = 0; i < 1; i++) {
			for (int j = 0; j < w; j++) {
				B[i][j] = new ActiveCounter8();
			}
		}
		return B;
	}
	...
	public static void estimateSize(String filePath) throws FileNotFoundException {
		...
		Scanner sc = new Scanner(new File(filePath));
		String resultFilePath = GeneralUtil.path + "Results\\RCS-AC_" 
			...
			if (true) {
				...
				for(int i = 0; i < d; i++) {
					...
					//j =0;
					 ...
					 value[i] =  C[0][j].getValue(0);
					estimate += value[i];
					
				}
				estimate =(int)( (w*d)*1.0/(w-d)*(  estimate/d-n/w)   ) ;
	...
	public static double getnoise() {
		...
		for(int i=0;i<100;i++) {
			...
			for(int j=0;j<d;j++) {				
				...
				estimate = Math.min(estimate, C[0][p].getValue(0));
			}
			res+=(double)estimate;
		}
