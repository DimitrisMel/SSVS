...
public class VirtualVariableCounter16 {
	...
	public static void estimateSize(String filePath) throws FileNotFoundException {
		...
		Scanner sc = new Scanner(new File(filePath));
		String resultFilePath = GeneralUtil.path + "error\\VirtualVariableCounter16_" 
		...
		while (sc.hasNextLine()) {
			...
			if (true) {
				...
				for(int i = 0; i < d; i++) {
					...
					j = (j % w + w) % w;
					//j =0;
					int index = C[i][j].getI();
					int tempV;
					int delta =1;
					if ((hashV & 1) ==0) delta =-1;
					if ( (hashV &2)==0) tempV = delta * C[i][j].getValue(0);
					else tempV = delta * C[i][j].getValue(1);
					estimate += tempV;
				}
